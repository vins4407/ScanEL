...
const Testing = ({ vulnerability }) => {
    return (
        <div>
            {Object.entries(vulnerability.vulnerabilities).map(([vulnerability, details]) => (
                <div key={vulnerability}>
                    <h2>{vulnerability}</h2>
                    <p>Description: {details.desc}</p>
                    <p>Solution: {details.sol}</p>
                    <p>References:</p>
                    <ul>
                        {Object.entries(details.ref).map(([refName, refLink]) => (
                            <li key={refName}>
                                <a href={refLink}>{refName}</a>
                            </li>
                        ))}
                    </ul>
                </div>
...
const VulnData = ({ digdata }) => {
  return (
    <div className='mainDiv'>
      ...
      <table>
        <tbody>
          {digdata["Content Security Policy Configuration"].map((item, index) => (
            <tr key={index}>
              <td>Method:</td>
              <td>{item.method}</td>
              <td>Path:</td>
              ...
              <td>HTTP Request:</td>
              <td>{item.http_request}</td>
              <td>Curl Command:</td>
              <td>{item.curl_command}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
VulnData.propTypes = {
  digdata: PropTypes.object.isRequired,
};
{/* <VulnData digdata={JSON.parse(data.Vulnerability)}/> */}
...
const vulnerability = {
        ...
        "Content Security Policy Configuration": [
            {
                ...
                "curl_command": "curl \"https://dhan.co/\""
            },
            {
                "method": "GET",
                "path": "/",
    ...
    "infos": {
        "target": "https://dhan.co/",
        "date": "Sat, 03 Jun 2023 19:13:19 +0000",
